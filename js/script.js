window.addEventListener("load", () => {
    localStorage.getItem("listaTerrenos")
        ? recuperarListaTerrenos(listaTerrenos)
        : agregarTerreno(bd);
    mostrarPrincipal();
});
...
listaTerrenos = [
    {
        ...
        descripcion:
            "Magnífico cráter de 95km cuadrados, a metros de los restos del Luna 21. Ideal para familia numerosa. Espacio para quincho y parrilla"
    },

    {
        ...
        ubicacion: "Fabbroni",
        descripcion:
            "Pequeño y acogedor cráter de 10km cuadrados. Ubicado cerca de los restos de la misión Apollo 17. Date el gusto de fotografiar a la familia Armstrong!"
    },

    {
        ...
        descripcion:
            "Si vivir en un cráter no es lo tuyo te traemos ésta planicie de 92km cuadrados. Oportunidad para emprendedores cancha de Paddle."
    },

    {
        ...
        "descripcion":
            "Esta propiedad a precio regalado no tiene desperdicio, ubicada en el lado oscuro de la luna un lugar ideal para aquellos que son fotosensibles."
    }
]
let bd = [
    {
...
/**----------FUNCIONES----------------------------------------------------------------------------------------- */
function agregarTerreno(listaTerrenos) {
    //RENDERIZA UNA LISTA DE TERRENOS EN LA PAGINA PRINCIPAL
    let contenedor = document.getElementById("contenedor");
    contenedor.innerHTML = "";
    listaTerrenos.forEach((terreno) => {
        contenedor.innerHTML += `<div class="col col-xxl-2 col-xl-3 col-lg-4 col-md-4 col-sm-6 mt-3 tarjeta">
...
function cargarLista() {
    //TOMA LOS DATOS DEL FORMULARIO Y GUARDA LA NUEVA PROPIEDAD EN LA LISTA DE TERRENOS
    ...
    if (titulo_terreno.value.trim() == "") {
        titulo_terreno.setAttribute(
            "placeholder",
            "Este campo no puede quedar vacio!"
        );
    } else if (
        precio_terreno.value.trim() == "" ||
        Number.isNaN(precio_terreno.value)
    ) {
        precio_terreno.setAttribute(
            "placeholder",
            "Ingrese un valor numerico valido"
        );
    } else if (ubicacion_terreno.value.trim() == "") {
        ubicacion_terreno.setAttribute(
            "placeholder",
            "Este campo no puede quedar vacio!"
        );
    } else if (imagen_1_terreno.value.trim() == "") {
        imagen_1_terreno.setAttribute(
            "placeholder",
            "Este campo no puede quedar vacio!"
        );
    } else if (descripcion_terreno.value.trim() == "") {
        descripcion_terreno.setAttribute(
            "placeholder",
            "Este campo no puede quedar vacio!"
        );
    } else {
...
function ordenarMenorAmayor() {
    //ORDENA LAS PROPIEDADES PUBLICADAS
...
function mostrarInfo() {
    //MUESTRA LA SECCION QUIENES SOMOS
...
function mostrarConsulta() {
    //MUESTRA FORMULARIO DE CONSULTA (BOTON COMPRAR)
    contenedor.classList.add("oculta");
...
function MostrarPublicar() {
    //MUESTRA EL FORMULARIO PARA PUBLICAR PROPIEDADES
    contenedor.classList.add("oculta");
    form_contacto.classList.add("oculta");
...
function guardarDatosCliente() {
    //TOMA DATOS DEL FORM DE CONSULTAS Y LOS GUARDA EN LOCALSTORAGE
...
function devolverCliente(nombre, apellido) {
    //Funcion para testeo
    let listaClientes = JSON.parse(localStorage.getItem(`${nombre}${apellido}`));
    console.log(listaClientes);
}
function guardarListaTerrenos(listaTerrenos) {
    //GUARDA LA LISTA DE TERRENOS EN LOCALSTORAGE
    localStorage.setItem("listaTerrenos", JSON.stringify(listaTerrenos));
}
function recuperarListaTerrenos() {
    //RETORNA LA LISTA DE TERRENOS GUARDADA EN LOCALSTORAGE
...
let btn_publicar = document
    .getElementById("btn_publicar")
...
fetch('https://www.dolarsi.com/api/api.php?type=valoresprincipales')
    ...
    .then((dato) => {
        let dolarCompra = document.getElementById('compra')
        dolarCompra.innerText = dato[1].casa.compra;
        let dolarVenta = document.getElementById('venta')
        dolarVenta.innerText = dato[1].casa.venta;
    })
